        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div class="lg:col-span-8 bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8">
                <div class="flex items-center gap-2 text-xs text-gray-500 mb-4">
                    ...
                    <span>/</span>
                    <span class="font-bold text-gray-800"><?php echo esc_html(single_cat_title('', false)); ?></span>
                </div>
                ...
                $children = get_terms([
                    'taxonomy' => 'category',
                    'parent'   => isset($term->term_id) ? (int)$term->term_id : 0,
                    'hide_empty' => false
                ]);
                    ...
                    <div class="flex flex-wrap gap-2 mb-6">
                        <?php foreach ($children as $child) : ?>
                            <a href="<?php echo esc_url(get_term_link($child)); ?>"
                               class="px-3 py-1 bg-gray-100 text-gray-600 text-xs rounded-md hover:bg-gray-200 transition-colors <?php echo (get_queried_object_id() === $child->term_id) ? 'bg-blue-50 text-blue-700' : ''; ?>">
